...
const ARGV = yargs
    .string('input').demand('input')
    .string('output').demand('output')
    .argv;

(async () => {
    const swaps = (await fs.promises.readFile(ARGV.input, 'utf-8')).split('\n').filter(s => s).map(line => JSON.parse(line));
    for (const swap of swaps) {
        const { metadata } = swap;
        metadata.swapResult.gasUsed = Math.abs(metadata.swapResult.gasUsed);
    }
    await fs.promises.writeFile(ARGV.output, swaps.map(s => JSON.stringify(s)).join('\n') + '\n', 'utf-8');
