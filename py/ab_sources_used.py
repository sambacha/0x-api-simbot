    ...
    for url, swap in d.items():
        for source in swap['sources']:
            ...
            counts_by_source_by_url[source][url] = \
                counts_by_source_by_url[source].get(url, 0)
            counts_by_source_by_url[source][url] += 1
            totals_by_url[url] = totals_by_url.get(url, 0) + 1
            urls.add(url)
urls = sorted(urls)
sources = sorted(set(counts_by_source_by_url.keys()))

prev_ys = [0 for d in urls]
xs = list(range(len(urls)))
for s in sources:
    ys = [counts_by_source_by_url[s].get(u, 0) / totals_by_url[u] for u in urls]
    plt.bar(xs, ys, bottom=prev_ys, label=s)
    prev_ys = [py + y for py, y in zip(prev_ys, ys)]

plt.legend()
plt.xticks(list(range(len(urls))), urls)
plt.gca().yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, pos: '%d%%' % (y * 100)))
plt.xlabel('endpoint')
plt.ylabel('source inclusion rate')
plt.title(f'frequency of sources included by endpoint ({len(data)} swaps)')
plt.show()
